...
DATASET_PATH="gs://[YOUR_BUCKET]/datasets/VerMulti/mathv60k_message.jsonl"
PRETRAIN_MODEL_PATH="Qwen/Qwen2.5-VL-3B-Instruct"
SAVE_PATH="${GCS_BUCKET}/checkpoints"
...
if [ -z "${GCP_WORKER}" ]; then
    ...
    ray start --head --node-ip-address=0.0.0.0 --port=${RAY_PORT} --dashboard-port=${DASHBOARD_PORT} --num-gpus=${GPU_COUNT}
    ...
    TOTAL_GPUS=$((TOTAL_NODES * GPU_COUNT)) # Each node has GPU_COUNT GPUs
    VLLM_NUM_ENGINES=$((TOTAL_GPUS))  # Use GPUs for vLLM engines
    VLLM_TP=2  # Tensor parallel size such that VLLM_NUM_ENGINES * VLLM_TP = TOTAL_GPUS
    ...
    ray job submit --address="http://127.0.0.1:${DASHBOARD_PORT}" \
      ...
      -- python3 -m openrlhf.cli.train_ppo_ray \
         ...
         --ref_num_gpus_per_node ${GPU_COUNT} \
         --remote_rm_url "${REWARD_MODEL_URL}" \
         --actor_num_nodes ${TOTAL_NODES} \
         --actor_num_gpus_per_node ${GPU_COUNT} \
         --vllm_num_engines ${TOTAL_GPUS} \
    ...
    GPU_COUNT=$(nvidia-smi -L | wc -l)
    ray start --address=${GCP_HEAD_IP}:${RAY_PORT} --num-gpus=${GPU_COUNT}
